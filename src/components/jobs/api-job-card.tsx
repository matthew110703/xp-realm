...
export function APIJobCard({ job, onClick }: Props) {
  const isPriority = job.jobType && JOB_TYPE_ACCENT.includes(job.jobType);

  return (
    <Card
      className={cn(
        "border-border/50 hover:border-primary/30 transition-all duration-200 cursor-pointer group",
      ...
      aria-label={`${job.title} at ${job.company ?? "Unknown"}`}
    >
